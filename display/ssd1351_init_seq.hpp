...
///
/// Stock chili-chip HAL used 0xF1 (max osc, divide-by-2). That halves the
/// panel refresh versus 0xF0 and is the main source of:
///   - rolling bars on phone cameras (PWM vs rolling shutter beat)
///   - flicker / eye strain when the handheld is moved
///
/// Override at compile time if a particular panel cannot tolerate /1:
///   -DSSD1351_CLOCK_DIV=0xF1
#ifndef SSD1351_CLOCK_DIV
#define SSD1351_CLOCK_DIV 0xF0
#endif

/// Command 0xC7 power-on master contrast (0–15). Brightness slider remaps this
/// at runtime; this is only the value programmed during init.
...
/// DISPLAY_ON is last so the first GRAM fill is not scanned as garbage.
